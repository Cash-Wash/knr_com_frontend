...
const navItems = [
  ...
  { label: "Emissions Web TV", icon: Tv2, href: "/admin/emissions", match: ["/admin/emissions"] },
...
export default function AdminSidebar({
  ...
  return (
    <>
      ...
      <aside
        className={[
          "fixed left-0 top-0 z-40 flex h-screen flex-col border-r border-slate-200 bg-white/95 transition-all duration-300 ease-in-out backdrop-blur-xl",
          isOpen ? "translate-x-0" : "-translate-x-full",
          isCollapsed ? "lg:w-20" : "lg:w-[272px]",
          "w-[272px] lg:translate-x-0",
        ].join(" ")}
      >
        <div className={["flex items-center border-b border-slate-200 py-4", isCollapsed ? "justify-center px-2" : "px-4"].join(" ")}>
          ...
          {!isCollapsed ? (
            <button
              type="button"
              onClick={onClose}
              className="ml-auto rounded-lg p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900 lg:hidden"
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-4 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-slate-100 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-300">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {!isCollapsed && item.badge ? (
                  <span className="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-sky-500/15 px-1.5 text-[10px] font-bold text-sky-600">
                    {item.badge}
                  </span>
                ) : null}
              </Link>
        ...
        <div className={["border-t border-slate-200 px-4 py-4", isCollapsed ? "space-y-4" : "space-y-3"].join(" ")}>
          <div className={["flex items-center", isCollapsed ? "justify-center" : "gap-3"].join(" ")}>
            ...
            {!isCollapsed ? (
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-slate-900">Admin KNR</p>
          ...
          <button
            ...
            className={[
              "flex w-full items-center rounded-xl text-sm font-medium text-red-500 transition-colors hover:bg-red-50 hover:text-red-600",
